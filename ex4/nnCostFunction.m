...
a1 = [ones(m, 1) X]'; % add column of 1s to X (a1 is 401 x 5000 since we transpose it)
z2 = Theta1 * a1;  %z2 is 25 x 5000
a2 = sigmoid(z2);  %a2 is also 25 x 5000

a2 = [ones(1, size(a2, 2)); a2]; %add row of 1s, because X is transposed (bias unit).
% a2 is now 26 x 5000

z3 = Theta2 * a2;  % z3 is 10 x 5000
a3 = sigmoid(z3)';  %signmoid(z3) is 10 x 5000. We transpose it so a3 is 5000 x 10

% Setup the Y vectors wich must just have a one in the correct lable
% position for neural networks.
...
% Calculate and add the regulariztion below
